...
/* Catching errors, as done in libjpeg's example.c (and already done by
 * code/renderer/tr_image_jpg.c) - a bad/non-JPEG levelshot must fail this
 * ONE load, not take the whole client down. Before this, R_JPGErrorExit
 * called ri.Error(ERR_FATAL, ...) unconditionally, so any malformed
 * levelshots/<map>.jpg (wrong format, truncated download, wrong extension)
 * crashed to desktop instead of falling back like every other image loader
 * in R_LoadImage's imageLoaders[] table. #41.
 */
