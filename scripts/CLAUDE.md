...
- `qconsole.log` lands under `fs_homepath`; `bench.sh` sets `fs_homepath=$PWD`
  so it writes into `/Applications/Quake3/baseq3/` (moved off `~/Desktop/`
  2026-09-04, old-mac-quake3#49 -- TCC silently denies a headless launch read
  access there on modern macOS, with nothing written to qconsole.log; then off
  `~/quake3-play/` onto the canonical `/Applications/Quake3` install, #51).
