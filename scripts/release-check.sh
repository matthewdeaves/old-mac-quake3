...
for m in "${MACHINES[@]}"; do
  ...
  GOT=$(ssh -n -o ConnectTimeout=8 "$m" \
    'f=/Applications/Quake3/ioquake3.app/Contents/Info.plist; [ -f "$f" ] && grep -A1 CFBundleVersion "$f" | tail -1 | sed "s/.*<string>//;s|</string>.*||"' 2>/dev/null)
  if [ "$GOT" = "$VERSION" ]; then
  ...
  if ssh -n -o ConnectTimeout=8 "$m" 'test -f /Applications/Quake3/ioquake3' 2>/dev/null; then
